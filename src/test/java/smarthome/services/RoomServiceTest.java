...
class RoomServiceTest {
    ...
    @Test
    @DisplayName("Can´t save room return exception")
    void cantSaveRoomException() {
        Room room = new Room(null, "Classroom", 1, 3.0, 2.5, 2.0);
        RoomDetailDTO roomDTO = new RoomDetailDTO(null, "Classroom", 1, 3.0, 2.5, 2.0);
        NullPointerException e = new NullPointerException();
        when(roomRepository.save(room)).thenThrow(e);
        Assertions.assertThrows(NullPointerException.class, () -> roomService.save(roomDTO));
    }
    ...
    @Test
    @DisplayName("Set a house grid to a room")
    void setHouseGrid() {

        Room room = new Room("B108", "Classroom", 2, 2, 3, 1.5);
        HouseGrid grid = new HouseGrid(1L, "Grid1");

        when(roomRepository.findById("B108")).thenReturn(Optional.of(room));
        when(gridRepository.findById(1L)).thenReturn(Optional.of(grid));

        roomService.setHouseGrid("B108", 1L);

        String result = room.getHouseGrid().getDesignation();

        assertEquals("Grid1",result);
    }
